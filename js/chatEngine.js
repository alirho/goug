...
class ChatEngine extends EventEmitter {
    ...
    async sendMessage(userInput) {
        ...
        this.emit('message', modelMessage); // Add an empty model message to start streaming into

        try {
            await streamGeminiResponse(
                this.apiKey,
                this.messages.slice(0, -1), // Send history without the latest model message shell
                userInput,
                (chunk) => {
                    fullResponse += chunk;
            ...
            // Once streaming is done, update the last message and save
