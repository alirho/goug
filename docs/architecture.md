...
|      [Peik Class] <----> [Chat Logic] <----> [Plugin Mgr]     |
|                           ^     |                             |
|                           |     v                             |
|                     [Event Emitter]                           |
+-----------------------------+---------------------------------+
...
### لایه اول: هسته (Core)
این لایه مغز متفکر سیستم است و هیچ وابستگی خارجی ندارد.
-   **`Peik`**: کلاس اصلی و مدیریت‌کننده چرخه حیات.
-   **`Chat`**: مدیریت وضعیت و منطق یک گفتگو.
-   **`PluginManager`**: مسئول ثبت، نصب و مدیریت وابستگی افزونه‌ها.
-   **`EventEmitter`**: سیستم ارتباطی داخلی.
...
3.  **Provider Plugin**: هسته بر اساس تنظیمات، افزونه مناسب (مثلاً Gemini) را پیدا کرده و پیام‌ها را به آن می‌سپارد.
4.  **Network Plugin**: افزونه Provider از طریق `HttpClient` درخواست را به سرور API می‌فرستد.
5.  **Streaming**: پاسخ‌ها به صورت تکه‌تکه (Chunk) دریافت شده و از طریق رویداد `chunk` به هسته و سپس به UI می‌رسند.
6.  **Storage Plugin**: پس از تکمیل پاسخ، وضعیت جدید گپ به صورت خودکار در دیتابیس ذخیره می‌شود.
...
-   **مدیریت خطا:** خطاها در سطح افزونه به خطاهای استاندارد (`PeikError`) تبدیل شده و به هسته منتقل می‌شوند تا مدیریت یکپارچه صورت گیرد.