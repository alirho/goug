...
-   **مسئولیت‌ها**:
    ...
    -   **تزریق وابستگی (Dependency Injection)**: دریافت یک **آداپتور ذخیره‌سازی** و مجموعه‌ای از **آداپتورهای ارائه‌دهنده** در `constructor` خود.
    -   فراخوانی متدهای آداپتور ذخیره‌سازی (`this.storage.saveChat()`, `this.storage.loadAllChats()`).
    -   استفاده از **الگوی استراتژی (Strategy Pattern)** برای انتخاب `Provider` مناسب. این ارائه‌دهنده‌ها به صورت پویا ثبت می‌شوند و هسته هیچ وابستگی مستقیمی به آن‌ها ندارد. (به [راهنمای آداپتور ارائه‌دهنده](./providerAdaptorGuide.md) مراجعه کنید).
